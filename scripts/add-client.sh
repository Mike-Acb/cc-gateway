...
# Pre-approve client token so Claude Code uses it as x-api-key
# instead of falling back to OAuth (required since v2.1.90+)
TRUNCATED_KEY=$(echo -n "$CLIENT_TOKEN" | tail -c 20)
...
with open(config_path, 'r') as f:
    config = json.load(f)
resp = config.setdefault('customApiKeyResponses', {'approved': [], 'rejected': []})
changed = False
if key in resp.get('rejected', []):
    resp['rejected'].remove(key)
    changed = True
if key not in resp.get('approved', []):
    resp.setdefault('approved', []).append(key)
    changed = True
if changed:
